...
{
  public:
    /** Create a SDP6x object with I2C address 0x40, //(64).
     */
    SDP6x(I2C &i2c, char i2c_address = 0x40);
    virtual ~SDP6x();
  ...
  private:
    I2C &i2c;
    char address;
    bool readSensor(char command, uint16_t* res);
    PRES_SENSOR_ERROR CheckCrc(char data[], char nbrOfBytes, char checksum);
